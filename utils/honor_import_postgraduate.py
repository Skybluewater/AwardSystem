...
competition_table = open_workbook("../excels/award_postgraduate.xlsx").sheet_by_index(6)
...
for row in range(1, competition_table.nrows):
    line = competition_table.row_values(row)
    user = User.objects.get(student_id=line[0], isPostgraduate=True)
    name = line[10]
    prize = competeChoice[line[11]]
    ...
    HonorTitle.objects.create(User=user, SchoolYear=schoolYear, honorName=name, proclaimOrganization=proclaimUnit, honorRank=prize, issueDate=date)
